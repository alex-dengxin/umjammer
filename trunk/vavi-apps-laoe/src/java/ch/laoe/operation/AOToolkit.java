...
/**
 * this toolkit contains common procedures, math and algorithms.
 * 
 * @autor olivier gäumann, neuchâtel (switzerland)
 * @target JDK 1.3
 * @version 21.04.01 first draft oli4
 */
public class AOToolkit {
    ...
    public static float interpolate3(float[] data, float index) {
        try {
            ...
            /*
             * //3rd order spline interpolation int ip = (int)index; float fp = index - ip; float l1 = data[ip]; float l0 =
             * data[ip+1]; float h0 = data[ip+2]; float h1 = data[ip]+3;
             * 
             * return l0 + .5f * fp * (h0 - l1 + fp * (h0 + l0 * (-2) + l1 + fp * ((h0 - l0) * 9 + (l1 - h1) * 3 + fp * ((l0 - h0) *
             * 15 + (h1 - l1) * 5 + fp * ((h0 - l0) * 6 + (l1 - h1) * 2)))));
             * 
             */
        } catch (ArrayIndexOutOfBoundsException e) {
            return 0;
    ...
    /**
     * zeroth order interpolation. the x-values must be ordered in rising manner. x- and y-length must be equal.
     */
    ...
    /**
     * first order interpolation. the x-values must be ordered in rising manner. x- and y-length must be equal.
     */
    ...
    /**
     * second order interpolation. the x-values must be ordered in rising manner. x- and y-length must be equal.
     */
    ...
    /**
     * third order interpolation. the x-values must be ordered in rising manner. x- and y-length must be equal.
     */
    ...
    /**
     * creates a lowpass filter kernel. WARNING: right-to-left processing required.
     */
    public static void setLowPassKernel(float[] kernel, int m, float fc) {
        ...
        for (int i = 0; i < m; i++) {
            ...
            // blackman window...
            kernel[i] *= (0.42 - 0.5 * Math.cos(2 * Math.PI * i / m) + 0.08 * Math.cos(4 * Math.PI * i / m));
            summ += kernel[i];
        }
        // normalize...
        for (int i = 0; i < m; i++) {
    ...
    /**
     * creates a highpass filter kernel. WARNING: right-to-left processing required. this kernel doesn't work correctly yet!!!
     */
     ...
     */
    public static void addIirBandPass(float[] x, float[] y, int o, int l, float f, float q, float gain) {
        // coefficients
     ...
     */
    public static void setIirHighPass(float[] data, int o, int l, float dry, float wet, float freq) {
        // coefficients
     ...
     */
    public static void setIirLowPass(float[] data, int o, int l, float dry, float wet, float freq) {
        // coefficients
        float omega = (float) (2. * Math.PI * freq);
        float cs = (float) Math.cos(omega);
        float alpha = (float) (Math.sin(omega) / 2.);
     ...
     */
    public static void setIirNotch(float[] data, int o, int l, float freq, float q, float gain) {
        // coefficients
        float omega = (float) (2. * Math.PI * freq);
        float cs = (float) Math.cos(omega);
    ...
    private static void baseFft(boolean isInvers, float[] re, float[] im) {
        ...
        int mmax, istep;
        // GProgressViewer.entrySubProgress(new ProgressEvent(this));
        for (mmax = 1, istep = 2 * mmax; mmax < n; mmax = istep, istep = 2 * mmax) {
            // GProgressViewer.setProgress(new ProgressEvent(this, mmax * 100 / n);
        ...
        // GProgressViewer.exitSubProgress(new ProgressEvent(this));
    ...
    /**
     * return the nearest positive peak index from center or -1 if no peak found. the search ends after reaching length in both
     * directions.
     */
    ...
    /**
     * return the nearest negative peak index from center or -1 if no peak found. the search ends after reaching length in both
     * directions.
     */
    ...
    /**
     * return the nearest zero-cross index from center or -1 if no zero-cross found. the search ends after reaching length in both
     * directions.
     */
    ...
    /**
     * return the number of samples greater than silenceThreshold in the given range.
     */
    public static int getNumberOfNoiseSamples(float[] s, int offset, int length, float silenceThreshold) {
        int counter = 0;
    ...
    /**
     * return the number of samples lower than silenceThreshold in the given range.
     */
    public static int getNumberOfSilenceSamples(float[] s, int offset, int length, float silenceThreshold) {
        int counter = 0;
    ...
    /**
     * return the index of the begin of the next noise-area on right side, or -1 if no noise found. noise is defined by threshold
     * and minimum length.
     */
    public static int getNextUpperNoiseIndex(float[] s, int offset, int length, float silenceThreshold, int minWidth) {
        int widthCounter = 0;
    ...
    /**
     * return the index of the begin of the next noise-area on left side, or -1 if no noise found. noise is defined by threshold
     * and minimum length.
     */
    public static int getNextLowerNoiseIndex(float[] s, int offset, int length, float silenceThreshold, int minWidth) {
        int widthCounter = 0;
    ...
    /**
     * return the index of the begin of the next silence-area on right side, or -1 if no silence found. silence is defined by
     * threshold and minimum length.
     */
    public static int getNextUpperSilenceIndex(float[] s, int offset, int length, float silenceThreshold, int minWidth) {
        int widthCounter = 0;
    ...
    /**
     * return the index of the begin of the next silence-area on left side, or -1 if no silence found. silence +++++++++++++++++
     * is defined by threshold and minimum length.
     */
    public static int getNextLowerSilenceIndex(float[] s, int offset, int length, float silenceThreshold, int minWidth) {
        int widthCounter = 0;
    ...
    public static AChannelSelection expandSilence(AChannelSelection s, float silenceThreshold, int minimumWidth) {
        ...
        low = AOToolkit.getNextLowerNoiseIndex(ch.sample, low, low, silenceThreshold, minimumWidth);
        high = AOToolkit.getNextUpperNoiseIndex(ch.sample, high, ch.sample.length - high, silenceThreshold, minimumWidth);
    ...
    public static AChannelSelection expandNoise(AChannelSelection s, float silenceThreshold, int minimumWidth) {
        ...
        low = AOToolkit.getNextLowerSilenceIndex(ch.sample, low, low, silenceThreshold, minimumWidth);
        high = AOToolkit.getNextUpperSilenceIndex(ch.sample, high, ch.sample.length - high, silenceThreshold, minimumWidth);
    ...
    public static AChannelSelection reduceSilence(AChannelSelection s, float silenceThreshold, int minimumWidth) {
        ...
        low = AOToolkit.getNextUpperSilenceIndex(ch.sample, low, ch.sample.length - low, silenceThreshold, minimumWidth);
        high = AOToolkit.getNextLowerSilenceIndex(ch.sample, high, high, silenceThreshold, minimumWidth);
    ...
    public static AChannelSelection reduceNoise(AChannelSelection s, float silenceThreshold, int minimumWidth) {
        ...
        low = AOToolkit.getNextUpperNoiseIndex(ch.sample, low, ch.sample.length - low, silenceThreshold, minimumWidth);
        high = AOToolkit.getNextLowerNoiseIndex(ch.sample, high, high, silenceThreshold, minimumWidth);
    ...
    /**
     * applies a zero-cross boundary: filters clipping, which occur on editing (copy, paste, cut...) and some effects, when the
     * phase of the signal makes a step. works in circular mode (end-begin)
     * 
    ...
    public static void applyZeroCross(float[] sample, int crossIndex, int crossWidth) {
        try {
             ...
             * 
             * if (effectiveCrossWidth > 0) { float f0 = 1.f / effectiveCrossWidth; float mean = average(sample,
             * crossIndex-crossWidth/2, crossWidth);
             * 
             * for (int i=0; i<effectiveCrossWidth; i++) { float f = f0 * i; int iLeft = (crossIndex - i + sample.length) %
             * sample.length; int iRight = (crossIndex + i) % sample.length;
             * 
             * sample[iLeft] = sample[iLeft] * f + mean * (1 - f); sample[iRight] = sample[iRight] * f + mean * (1 - f); } }
             */
     ...
     * returns a new Fifo
     */
    public static AOFifo createFifo(int maxSize) {
        return new AOFifo(maxSize);
    }

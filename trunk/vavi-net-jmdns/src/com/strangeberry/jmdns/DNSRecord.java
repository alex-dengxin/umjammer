...
abstract class DNSRecord extends DNSEntry {
    ...
    /**
     * True if this record would be supressed by an answer. This is the case if this record would not have a significantly longer
     * TTL.
     */
    ...
    static class Address extends DNSRecord {
        ...
        /**
         * Does a lexicographic comparison of the byte array representation of this record and that record. This is needed for
         * tie-break tests according to draft-cheshire-dnsext-multicastdns-04.txt chapter 9.2.
         */
            ...
            if (dnsAddress != null) {
                if (dnsAddress.sameType(this) && dnsAddress.sameName(this) && (!dnsAddress.sameValue(this))) {
                    ...
                    if (dns.getState().isProbing() && lexCompare(dnsAddress) >= 0) {
                        ...
                        dns.getCache().clear();
                        for (Iterator<ServiceInfo> i = dns.services.values().iterator(); i.hasNext();) {

 ...
 * 
 * 
 * Class: GControlTextF @author olivier gäumann, neuchâtel (switzerland) @target JDK 1.3
 * 
 * UiControlText with units adapted to x-axis of a clip and the frequency unit.
 * 
 * @version 28.04.01 first draft oli4
 * 
 */
public class GControlTextF extends UiControlText {
    public GControlTextF(GMain main, int digits, boolean incrementVisible, boolean unitVisible) {
        super(digits, incrementVisible, unitVisible);
        this.main = main;
        setUnit(" ");
    }

    protected GMain main;
    ...
    protected double dataToUnit(double d) {
        ...
        case 4:
            return main.getFocussedClipEditor().getClip().getSampleRate() / d;
    ...
    protected double unitToData(double u) {
        ...
        case 4:
            return 1 / u * main.getFocussedClipEditor().getClip().getSampleRate();

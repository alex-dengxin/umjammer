...
public class JmDNS {
     ...
     * @see #list
     */
    private Map<String, ServiceCollector> serviceCollectors = new HashMap<String, ServiceCollector>();
    ...
    private void init(InetAddress address, String name) throws IOException {
        ...
        services = new HashMap<String, ServiceInfo>(20);
        serviceTypes = new HashMap<String, String>(20);
    ...
    private void start(Collection<ServiceInfo> serviceInfos) {
        ...
        for (Iterator<ServiceInfo> iterator = serviceInfos.iterator(); iterator.hasNext();) {
            try {
                registerService(new ServiceInfo(iterator.next()));
    ...
    void handleServiceResolved(ServiceInfo info) {
        ...
        if (list != null) {
            ...
            final List<ServiceListener> listCopy = new ArrayList<ServiceListener>(list);
     ...
     * clients. The name of the service may be changed to make it unique.
     */
     ...
     */
    protected boolean makeHostNameUnique(DNSRecord.Address host) {
    ...
    /**
     * The Prober sends three consecutive probes for all service infos that
     * needs probing as well as for the host name. The state of each service
     * info of the host name is advanced, when a probe has been sent for it.
     ...
     * affected host name) are taken away from the prober. This eventually
     * causes the prober tho cancel itself.
     */
    private class Prober extends TimerTask {
        ...
        public boolean cancel() {
            ...
            synchronized (JmDNS.this) {
                for (Iterator<ServiceInfo> i = services.values().iterator(); i.hasNext();) {
     ...
     * the state of all serviceInfos, for which it has sent an announce. The
     * Announcer also sends announcements and advances the state of JmDNS
     * itself.
     * 
    ...
    /**
     * The Responder sends a single answer for the specified service infos and
     * for the host name.
     */
    private class Responder extends TimerTask {
        ...
        public void start() {
            ...
            for (Iterator<? extends DNSEntry> i = in.questions.iterator(); i.hasNext();) {
            ...
            synchronized (ioLock) {
                ...
                if (state == DNSState.ANNOUNCED) {
                    try {
                        ...
                        // Answer questions
                        for (Iterator<? extends DNSEntry> iterator = in.questions.iterator(); iterator.hasNext();) {
                            ...
                            if (entry instanceof DNSQuestion) {
                                ...
                                case DNSConstants.TYPE_PTR: {
                                    // Answer a query for services of a given
                                    // type
                                ...
                                default: {
                                    // System.out.println("JmDNSResponder.unhandled
                                    // query:"+q);
                                    break;
                                }
                                }
    ...
    /**
     * The ServiceResolver queries three times consecutively for services of a
     * given type, and then removes itself from the timer.
     * 
     ...
     * service info, and then removes itself from the timer.
     * 
    ...
    private class Canceler extends TimerTask {
        ...
        /**
         * The services that need cancelling. Note: We have to use a local
         * variable here, because the services that are canceled, are removed
         * immediately from variable JmDNS.services.
         */
        private ServiceInfo[] infos;

        /**
         * We call notifyAll() on the lock object, when we have canceled the
         * service infos. This is used by method JmDNS.unregisterService() and
         * JmDNS.unregisterAllServices, to ensure that the JmDNS socket stays
         * open until the Canceler has canceled all services.
         * 
...
//      private static Logger logger = Logger.getLogger(ServiceCollector.class.toString());
        ...
        public ServiceInfo[] list() {
            synchronized (infos) {
                return infos.values().toArray(new ServiceInfo[infos.size()]);
            }
        }
